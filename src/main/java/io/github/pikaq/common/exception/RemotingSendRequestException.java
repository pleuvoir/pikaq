package io.github.pikaq.common.exception;

public class RemotingSendRequestException extends Exception {

	private static final long serialVersionUID = 1523512974133417443L;

	public RemotingSendRequestException() {
		super();
	}

	public RemotingSendRequestException(String message, Throwable cause) {
		super(message, cause);
	}

	public RemotingSendRequestException(String message) {
		super(message);
	}

	public RemotingSendRequestException(Throwable cause) {
		super(cause);
	}

}
